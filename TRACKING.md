...
- 用户打开 http://47.82.182.96 进入 Web 聊天界面
...
主窗口（默认）
  │
  ├─ "皇帝游戏那个bug..."  →  自动检测到 emperor-game 项目
  │                            → 分叉：创建/切换到皇帝游戏窗口
  │                            → 相关消息转移到该窗口
  │
  ├─ "长电财报数据..."    →  自动检测到 earnings 项目
  │                            → 分叉到财报窗口
  │
  └─ "新想法：做个..."     →  检测到新主题
                               → 提示：是否创建新项目？
...
│   ├── MEMORY.md        # 项目专属记忆
│   ├── skills/          # 项目专属技能
│   └── workspace → /path/to/game  # 软链接到实际代码
├── redmane/
├── earnings/
├── morning-brief/
└── podcast/
...
- [x] Web Demo 上线：http://47.82.182.96/projects/ai-agent-context-demo/
- [x] 主页添加入口卡片
- [x] 工作流优化：「先搜再做」写入记忆
